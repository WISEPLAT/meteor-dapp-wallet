updateMistBadge = function(){
    ...
    if(typeof mist !== 'undefined' && (!conf || !conf.confirmedOwners.length)) {
        var accounts = WshAccounts.find({}).fetch();
        ...
        mist.menu.setBadge(WshTools.formatBalance(balance, '0.0 a','wise') + ' WSH');
    }
};
...
updateMistMenu = function(){
    ...
    var accounts = _.union(Wallets.find({}, {sort: {name: 1}}).fetch(), WshAccounts.find({}, {sort: {name: 1}}).fetch());
    ...
    Meteor.setTimeout(function(){
        ...
        _.each(accounts, function(account, index){
            mist.menu.add(account._id,{
                position: 3 + index,
                name: account.name,
                badge: WshTools.formatBalance(account.balance, "0 a", 'wise')+ ' WSH',
