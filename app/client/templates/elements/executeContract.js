...
Template['elements_executeContract'].onCreated(function(){
    ...
    web3.wsh.getCode(template.data.address, function(e, code) {
        if(!e && code.length > 2) {
...
Template['elements_executeContract'].helpers({
    ...
    */
    'reactiveContext': function() {
        var contractInstance = web3.wsh.contract(this.jsonInterface).at(this.address);
        ...
        if(selectedAccount) {
            ...
            var sendTransaction = function(estimatedGas){
                ...
                if(contracts['ct_'+ selectedAccount._id]) {
                    ...
                    var accounts = WshAccounts.find({name: {$exists: true}}, {sort: {name: 1}}).fetch();
