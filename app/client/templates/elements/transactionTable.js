...
@type Number
*/
var blocksForConfirmation = wiseplatConfig.requiredConfirmations;
    ...
    'items': function(){
        ...
        if(searchQuery) {
            ...
            items = _.filter(items, function(item){
                ...
                // search value
                if(pattern.test(WshTools.formatBalance(item.value, '0,0.00[000000] unit')))
                    return item;
...
Template['elements_transactions_row'].helpers({
    ...
    */
    'incomingTx': function(account){
        var account = WshAccounts.findOne({_id: account}) || Wallets.findOne({_id: account});
        return !!((account && this.from !== account.address) ||
                  (!account && (WshAccounts.findOne({address: this.to}) || Wallets.findOne({address: this.to}))));
    },
    /**
    ...
    */
    'unConfirmed': function() {
        if(!this.blockNumber || !WshBlocks.latest.number)
            return {
                confirmations: 0,
                percent: 0
            };

        var currentBlockNumber = WshBlocks.latest.number + 1,
