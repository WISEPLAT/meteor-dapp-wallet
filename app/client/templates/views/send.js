...
var checkOverDailyLimit = function(address, wei, template){
    ...
    if(account && account.requiredSignatures > 1 && !_.isUndefined(account.dailyLimit) && account.dailyLimit !== wiseplatConfig.dailyLimitDefault && Number(wei) !== 0) {
        // check whats left
        ...
        if(restDailyLimit.lt(new BigNumber(wei, 10)))
            TemplateVar.set('dailyLimitText', new Spacebars.SafeString(TAPi18n.__('wallet.send.texts.overDailyLimit', {limit: WshTools.formatBalance(restDailyLimit.toString(10)), total: WshTools.formatBalance(account.dailyLimit), count: account.requiredSignatures - 1})));
        else
            TemplateVar.set('dailyLimitText', new Spacebars.SafeString(TAPi18n.__('wallet.send.texts.underDailyLimit', {limit: WshTools.formatBalance(restDailyLimit.toString(10)), total: WshTools.formatBalance(account.dailyLimit)})));
    } else
...
var getDataField = function(){
    ...
    if(selectedToken && selectedToken !== 'wise') {
...
Template['views_send'].onCreated(function(){
    ...
    if(FlowRouter.getRouteName() === 'deployContract') {
        ...
        TemplateVar.set('selectedToken', 'wise');

    // Send funds
    } else {
        TemplateVar.set('selectedAction', 'send-funds');
        TemplateVar.set('selectedToken', FlowRouter.getParam('token') || 'wise');
    }

    // check if we are still on the correct chain
    Helpers.checkChain(function(error) {
        if(error && (WshAccounts.find().count() > 0)) {
    ...
    template.autorun(function(c){
        ...
        if(!c.firstRun && TemplateVar.get('selectedToken') === 'wise') {
            TemplateVar.set('amount', WshTools.toWei(template.find('input[name="amount"]').value.replace(',','.'), unit));
        }
    });
...
Template['views_send'].onRendered(function(){
    ...
    template.autorun(function(c){
        ...
        // Wise tx estimation
        if(tokenAddress === 'wise') {

            if(WshAccounts.findOne({address: address}, {reactive: false})) {
                web3.wsh.estimateGas({
                    from: address,
                    to: to,
                    value: amount,
                    data: data,
...
Template['views_send'].helpers({
    ...
    */
    'total': function(wise){
        ...
        if (TemplateVar.get('selectedToken') === 'wise') {
    ...
    */
    ...
    /**
    Returns the total amount - the fee paid to send all wise/coins out of the account
...
Template['views_send'].events({
    ...
    */
    ...
    @event click .token-wise
    */
    'click .token-wise': function(e, template){
        TemplateVar.set('selectedToken', 'wise');
    ...
    */
    'keyup input[name="amount"], change input[name="amount"], input input[name="amount"]': function(e, template){
        // wise
        if(TemplateVar.get('selectedToken') === 'wise') {
            var wei = WshTools.toWei(e.currentTarget.value.replace(',','.'));
    ...
    */
    'submit form': function(e, template){
        ...
        if(selectedAccount && !TemplateVar.get('sending')) {
            ...
            if(sendAll && (selectedAccount.owners || tokenAddress !== 'wise'))
                sendAll = false;
            ...
            if(selectedAccount.balance === '0' && (!selectedAccount.owners || tokenAddress === 'wise'))
                return GlobalNotification.warning({
