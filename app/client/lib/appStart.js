...
web3.wsh.isSyncing(function(error, syncing) {
    if(!error) {
...
var connect = function(){
    ...
    if(web3.isConnected()) {
        ...
        // only start app operation, when the node is not syncing (or the wsh_syncing property doesn't exists)
        web3.wsh.getSyncing(function(e, sync) {
            if(e || !sync) {
                connectToNode();
            } else {
                WshAccounts.init();
        ...
        Meteor.setTimeout(function(){
            // if in mist, tell to start gwsh, otherwise start with RPC
            var gwshRPC = (web3.admin) ? 'gwsh' : 'gwsh --rpc --rpccorsdomain "'+window.location.protocol + '//' + window.location.host+'"';
