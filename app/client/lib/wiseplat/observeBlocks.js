...
updateBalances = function() {
    ...
    _.each(creatingWallets, function(wallet){
      ...
      web3.wsh.getTransactionReceipt(wallet.transactionHash, function(error, receipt) {
    ...
    // UPDATE ENS
    var allAccounts = WshAccounts.find().fetch().concat(walletsAndContracts);
        ...
        if (!account.ensCheck || (account.ensCheck && now - account.ensCheck > 10*60*1000)) {
            Helpers.getENSName(account.address, function(err, name, returnedAddr) {
                ...
                if (!err && account.address.toLowerCase() == returnedAddr){
                    WshAccounts.update({address: account.address}, {$set:{ name: name, ens: true, ensCheck: now}});
                    ...
                    WshAccounts.update({address: account.address}, {$set:{ens: false, ensCheck: now}});
    ...
    var walletsContractsAndAccounts = WshAccounts.find().fetch().concat(Wallets.find().fetch());
...
observeLatestBlocks = function(){
    ...
    web3.wsh.filter('latest').watch(function(e, res){
        if(!e) {
            updateBalances();
        }
    });
