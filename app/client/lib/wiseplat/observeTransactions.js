...
addTransactionAfterSend = function(txHash, amount, from, to, gasPrice, estimatedGas, data, tokenId) {
    ...
    // add from Account
    WshAccounts.update({address: from}, {$addToSet: {
        transactions: txId
    }});

    // add to Account
    WshAccounts.update({address: to}, {$addToSet: {
        transactions: txId
    }});
};
...
addTransaction = function(log, from, to, value){
    ...
    var block = web3.wsh.getBlock(log.blockNumber, false, function(err, block){
        if(!err) {

            web3.wsh.getTransaction(log.transactionHash, function(err, transaction) {
                if(!err && transaction) {
                    web3.wsh.getTransactionReceipt(log.transactionHash, function(err, receipt){
...
var updateTransaction = function(newDocument, transaction, receipt){
    ...
    if(newDocument.outOfGas) {
        ...
        if(WshAccounts.findOne({address: newDocument.from})) {
            web3.wsh.getBalance(newDocument.from, newDocument.blockNumber, function(e, now){
                if(!e) {
                    web3.wsh.getBalance(newDocument.from, newDocument.blockNumber-1, function(e, then){
    ...
    var checkTransactionConfirmations = function(tx){
        ...
        if(!tx.confirmed && tx.transactionHash) {
            ...
            var updateTransactions = function(e, blockHash){
                ...
                if(!e) {
                    var confirmations = (tx.blockNumber && WshBlocks.latest.number) ? (WshBlocks.latest.number + 1) - tx.blockNumber : 0;
                    confCount++;
                    ...
                    if(confirmations < wiseplatConfig.requiredConfirmations && confirmations >= 0) {
                        ...
                        web3.wsh.getTransaction(tx.transactionHash, function(e, transaction){
                            web3.wsh.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                    ...
                    if(confirmations > wiseplatConfig.requiredConfirmations || confCount > wiseplatConfig.requiredConfirmations*2) {
                            ...
                            web3.wsh.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                                if(!e) {
                                    ...
                                    if(!transaction || !transaction.blockNumber) {
                                        ...
                                        web3.wsh.getBlock(transaction.blockNumber, function(e, block) {
                                            if(!e) {
            ...
            var filter = web3.wsh.filter('latest').watch(function(e, blockHash) {
    ...
    collectionObservers[collectionObservers.length] = Transactions.find({}).observe({
        ...
        */
        added: function(newDocument) {
            var confirmations = WshBlocks.latest.number - newDocument.blockNumber;
            ...
            if( Session.get('network') == 'main' && 
               ...
               !newDocument.exchangeRates.brl)) {
                var url = 'https://min-api.cryptocompare.com/data/pricehistorical?fsym=WSH&tsyms=BTC,USD,EUR,GBP,BRL&ts='+ newDocument.timestamp;
        ...
        */
        removed: function(document) {
            ...
            WshAccounts.update({address: document.from}, {$pull: {
                transactions: document._id
            }});
            WshAccounts.update({address: document.to}, {$pull: {
                transactions: document._id
            }});
        }
    });
