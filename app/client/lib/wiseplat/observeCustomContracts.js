...
//     var contractInstance = web3.wsh.contract(newDocument.jsonInterface).at(newDocument.address);
//     var blockToCheckBack = (newDocument.checkpointBlock || 0) - wiseplatConfig.rollBackBy;
...
//     var currentBlock = WshBlocks.latest.number;
//     filter.get(function(error, logs) {
//         if(!error) {
//             // update last checkpoint block
//             CustomContracts.update({_id: newDocument._id}, {$set: {
//                 checkpointBlock: (currentBlock || WshBlocks.latest.number) - wiseplatConfig.rollBackBy
//             }});
//         }
//     });
...
//                 Events.remove(id);
//             } else {
//                 web3.wsh.getBlock(log.blockHash, function(err, block){
//                     if(!err) {
    ...
    collectionObservers[collectionObservers.length] = CustomContracts.find({}).observe({
        ...
        */
        added: function(newDocument) {
            ...
            web3.wsh.getCode(newDocument.address, function(e, code) {
                if(!e && code && code.length > 2 ){
                    ...
                    web3.wsh.getBalance(newDocument.address, function(e, balance) {
                        if(!e && balance.gt(0)){
                            CustomContracts.update(newDocument._id, {$unset: {
                                disabled: false
                            }});
