...
/**
Check if currenct unit is an wise unit

@method (isWiseUnit)
**/
Template.registerHelper('isWiseUnit', function(){
...
Template.registerHelper('selectAccounts', function(hideWallets){
    var accounts = WshAccounts.find({balance:{$ne:"0"}}, {sort: {balance: 1}}).fetch();
    
    if(hideWallets !== true)
        accounts = _.union(Wallets.find({owners: {$in: _.pluck(WshAccounts.find().fetch(), 'address')}, address: {$exists: true}}, {sort: {name: 1}}).fetch(), accounts);
...
Formats a given transactions balance
    ...
    {{formatTransactionBalance value exchangeRates "wise"}}
