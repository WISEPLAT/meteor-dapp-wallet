...
Helpers.getDefaultContractExample = function(withoutPragma) {
    ...
    if (withoutPragma) {
        ...
        // so that older versions that will query code from wallet.wiseplat.org won't use broken example code.
...
Helpers.getOwnedAccountFrom = function(accountList){
    ...
    var accounts = WshAccounts.find({}, {sort: {balance: 1}}).fetch();
...
Helpers.formatNumberByDecimals = function(number, decimals){
    ...
    return WshTools.formatNumber(new BigNumber(number, 10).dividedBy(Math.pow(10, decimals)), numberFormat);
};
...
Helpers.checkChain = function(callback){
    ...
    web3.wsh.getCode(originalContractAddress, function(e, code){
...
Helpers.isWatchOnly = function(id) {
    return !Wallets.findOne({_id: id, owners: {$in: _.pluck(WshAccounts.find({}).fetch(), 'address')}});
};
...
/**
Gets the docuement matching the given addess from the WshAccounts or Wallets collection.
...
Helpers.getAccountByAddress = function(address, reactive) {
    ...
    return WshAccounts.findOne({address: address}, options) || Wallets.findOne({address: address}, options) || CustomContracts.findOne({address: address}, options);
};

/**
Gets the docuement matching the given query from the WshAccounts or Wallets collection.
...
Helpers.getAccounts = function(query, reactive) {
    ...
    return WshAccounts.find(query, options).fetch().concat(Wallets.find(query, options).fetch());
};

/**
Gets the docuement matching the given addess from the WshAccounts or Wallets collection and returns its name or address.
...
Helpers.formatTransactionBalance = function(value, exchangeRates, unit) {
    ...
    var unit = unit || WshTools.getUnit(),
        format = '0,0.00';
    ...
    if((unit === 'usd' || unit === 'eur' || unit === 'btc') &&
       exchangeRates && exchangeRates[unit]) {
        ...
        var price = new BigNumber(String(web3.fromWei(value, 'wise')), 10).times(exchangeRates[unit].price);
        return WshTools.formatNumber(price, format) + ' '+ unit.toUpperCase();
    } else {
        return WshTools.formatBalance(value, format + '[0000000000000000] UNIT');
    }
};
...
Helpers.getENSName = function(address, callback) {
    ...
    var ensContract = web3.wsh.contract(ensContractAbi);
    var resolverContract = web3.wsh.contract(resolverContractAbi);
